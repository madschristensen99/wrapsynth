...
describe("Mint", () => {
  ...
  it("Sets and gets balance ciphertext", async () => {
    const owner = readKpJson(`${os.homedir()}/.config/solana/id.json`);
    ...
    // Set the balance
    console.log("Setting balance ciphertext...");
    const setSig = await program.methods
      .setBalance(Array.from(ciphertext[0]))
      .accountsPartial({
    ...
    console.log("Set balance signature:", setSig);

    // Get the balance
    console.log("Getting balance ciphertext...");
    const getResult = await program.methods
      .getBalance()
      .accountsPartial({
        user: owner.publicKey,
      })
    ...
    // Decrypt and verify
    const decrypted = cipher.decrypt([getResult as Array<number>], nonce)[0];
