...
\begin{frame}{\color{textprimary}\textbf{Fun Gerbil: Built on Arcium and Darklake}}
  ...
  \begin{tikzpicture}
    ...
    % DarkLake logo box - clipped image with frame
    \begin{scope}
      \clip[rounded corners=15pt] (-6.0,0) rectangle (-1.0,5.0);
      \node[inner sep=0pt] at (-3.5,2.5) {\includegraphics[width=5.0cm,height=5.0cm,keepaspectratio]{darklake.jpeg}};
    \end{scope}
    \draw[secondary,line width=2pt,rounded corners=15pt] (-6.0,0) rectangle (-1.0,5.0);
    
    % Arcium logo box - clipped image with frame
    \begin{scope}
      \clip[rounded corners=15pt] (1.0,0) rectangle (6.0,5.0);
      \node[inner sep=0pt] at (3.5,2.5) {\includegraphics[width=5.0cm,height=5.0cm,keepaspectratio]{arcium.jpg}};
    \end{scope}
    \draw[primary,line width=2pt,rounded corners=15pt] (1.0,0) rectangle (6.0,5.0);
  \end{tikzpicture}
...
\begin{frame}[t]{\color{textprimary}\textbf{Fun Gerbil: Cypherpunk Founders}}
  ...
  \begin{tikzpicture}
    % Larger white bubble to contain everything
    \fill[lightbg,rounded corners=20pt] (-7,-2.5) rectangle (7,4.0);
    \draw[textsecondary,line width=1pt,rounded corners=20pt] (-7,-2.5) rectangle (7,4.0);
    
    % MADS - Left founder box (taller to fit all content)
    \fill[midbg,rounded corners=8pt] (-6.5,-0.5) rectangle (-2.2,3.6);
    \node[text=textprimary,font=\bfseries,align=center] at (-4.35,3.3) {Mads Christensen};
    % Image with frame
    \begin{scope}
      \clip[rounded corners=4pt] (-5.35,1.0) rectangle (-3.35,3.0);
      \node[inner sep=0pt] (mads) at (-4.35,2.0) {\includegraphics[width=2.0cm,height=2.0cm,keepaspectratio]{mads.jpeg}};
    \end{scope}
    \draw[textsecondary,line width=1.5pt,rounded corners=4pt] (-5.35,1.0) rectangle (-3.35,3.0);
    \node[text=textsecondary,font=\small,align=center] at (-4.35,0.7) {CEO \& Founder};
    \node[text=accent,font=\scriptsize,align=center,text width=3.8cm] at (-4.35,0.0) {Monero Maximalist \\ Fullstack Engineer};
    
    % KYLE - Center founder box (taller to fit all content)
    \fill[midbg,rounded corners=8pt] (-1.8,-0.5) rectangle (1.8,3.6);
    \node[text=textprimary,font=\bfseries,align=center] at (0,3.3) {Kyle Koshiyama};
    % Image with frame
    \begin{scope}
      \clip[rounded corners=4pt] (-1.0,1.0) rectangle (1.0,3.0);
      \node[inner sep=0pt] (kyle) at (0,2.0) {\includegraphics[width=2.0cm,height=2.0cm,keepaspectratio]{kyle.jpeg}};
    \end{scope}
    \draw[textsecondary,line width=1.5pt,rounded corners=4pt] (-1.0,1.0) rectangle (1.0,3.0);
    \node[text=textsecondary,font=\small,align=center] at (0,0.7) {Protocol Engineer};
    \node[text=accent,font=\scriptsize,align=center,text width=3.2cm] at (0,0.0) {Former Fhenix\\Current DarkLake};
    
    % TIAGO - Right founder box (taller to fit all content)
    \fill[midbg,rounded corners=8pt] (2.2,-0.5) rectangle (6.5,3.6);
    \node[text=textprimary,font=\bfseries,align=center] at (4.35,3.3) {Tiago Alves};
    % Image with frame
    \begin{scope}
      \clip[rounded corners=4pt] (3.35,1.0) rectangle (5.35,3.0);
      \node[inner sep=0pt] (tiago) at (4.35,2.0) {\includegraphics[width=2.0cm,height=2.0cm,keepaspectratio]{tiago.jpg}};
    \end{scope}
    \draw[textsecondary,line width=1.5pt,rounded corners=4pt] (3.35,1.0) rectangle (5.35,3.0);
    \node[text=textsecondary,font=\small,align=center] at (4.35,0.7) {Researcher};
    \node[text=accent,font=\scriptsize,align=center,text width=3.8cm] at (4.35,0.0) {Cryptography PhD\\Current DarkLake};
    
    % Team strength statement - now properly inside the white bubble
    \node[text=primary,font=\large,align=center] at (0,-1.0) {\textbf{Deep Cryptography \& Privacy Expertise}};
    \node[text=textprimary,font=\normalsize,align=center] at (0,-1.6) {Connected across the Solana and Monero communities};
    \node[text=accent,font=\normalsize,align=center] at (0,-2.1) {Building the future of private, cross-chain DeFi};
  \end{tikzpicture}
\end{frame}
